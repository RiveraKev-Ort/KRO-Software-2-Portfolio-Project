...
For convention, I plan to use Camel case to make redability of the program easier, keep the naming and of such menthods and files descriptive and consistent. Use US currency with two decimal places for accuracy. Use ISO date format (YYYY-MM-DD) to help the scalibilty of the porgram for future development.

For Correspondence, use mapping to help keep categories grouped, enforce types such as floats or doubles to make operations accurate, and create validation rules such as zero based or variable income to denote if negative values are allowed or not.
